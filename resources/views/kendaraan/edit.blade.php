            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                ...
                <ul class="flex items-center gap-2 text-sm font-normal shrink-0">
                    <li
                        ...
                        <a href="{{ url('kendaraans') }}" class="text-slate-400 dark:text-zink-200">Kendaraan</a>
                    </li>
                ...
                <div class="card-body">
                    <form action="{{ url('kendaraans/' . $kendaraan->id . '') }}" class="mt-10" method="POST"
                            ...
                            <select
                                ...
                                @foreach ($client as $item)
                                    <option value="{{ $item->id }}-{{ $item->name }}"
                                        {{ $kendaraan->kendaraan_user_id == $item->id ? 'selected' : '' }}>
                                        {{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <select
                                ...
                                @foreach ($pool as $item)
                                    <option value="{{ $item->id }}-{{ $item->name }}"
                                        {{ $kendaraan->pool_id == $item->id ? 'selected' : '' }}>{{ $item->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <select
                                class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                data-choices="" name="status" id="status">
                                <option value="">Pilih Status</option>
                                <option value="0" {{ $kendaraan->status == 1 ? 'selected' : '' }}>Non Aktif</option>
                                <option value="2" {{ $kendaraan->status == 2 ? 'selected' : '' }}>Aktif</option>
                            </select>
                        </div>
                        <div class="mb-3">
...
@section('script')
    <script>
        ...
        // Fungsi untuk mereset pratinjau gambar dan nilai input file

        const uploadInput = document.getElementById('upload');
        const filenameLabel = document.getElementById('filename');
        const imagePreview = document.getElementById('image-preview');
        const kendaraanimage = document.getElementById('kendaraan_id');
